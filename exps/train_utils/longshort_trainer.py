...
class Trainer:
    ...
    def before_train(self):
        ...
        use_syncbn = getattr(self.exp, "use_syncbn", False)
        if use_syncbn:
    ...
    def update_ckpt(self, ckpt):
        ...
        for k, v in ckpt.items():
            if k.startswith("backbone"):
                ...
                if 'jian' in k:
                    res_ckpt[f'short_{k}'.replace("jian", "group_0_jian")] = v
                    res_ckpt[f'long_{k}'.replace("jian", "group_0_jian")] = v
            if k.startswith("neck"):
                res_ckpt[f'backbone.{k}'] = v

        ckpt_key_mapper = getattr(self.exp, "ckpt_key_mapper", None)
        if ckpt_key_mapper is None:
            return res_ckpt
        for k, v in ckpt.items():
            for k_ori in ckpt_key_mapper.keys():
                if k_ori in k:
                   res_ckpt[k.replace(k_ori, ckpt_key_mapper[k_ori])] = v
