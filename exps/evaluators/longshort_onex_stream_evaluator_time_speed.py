...
class LONGSHORT_ONEX_COCOEvaluator_TS:
    """
    ...
    def evaluate(
        ...
        counter_num = 500
        time_list = []
        for cur_iter, (short_imgs, long_imgs, _, info_imgs, ids) in enumerate(
                progress_bar(self.dataloader)
        ):
            with torch.no_grad():
                short_imgs = short_imgs.type(tensor_type)
                long_imgs = long_imgs.type(tensor_type)
                ...
                ### TODOlcy
                start_t = time.time()
                outputs = model((short_imgs, long_imgs))
                ...
                ### TODOlcy
                end_t = time.time()
                print(f"total: {(end_t-start_t)*1000} ms, inference: {(medium_t-start_t)*1000} ms, nms: {(end_t-medium_t)*1000} ms")
                if counter >= 200:
                    break
                time_list.append((end_t-start_t)*1000)
                counter += 1

            data_list.extend(self.convert_to_coco_format(outputs, info_imgs, ids))

        print(f"Average time: {sum(time_list[10:])/len(time_list[10:])} ms")
        import pdb
