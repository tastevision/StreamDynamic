# Refer to tal_flip_long_short_n_predictions_argoversedataset.py
# return three targets, use the targets of t and t-1 frames to calculate TAL weight. 
...
class LONGSHORT_xN_TAL_Ori_ARGOVERSEDataset(Dataset):
    """
    COCO dataset class.
    """
    ...
    def load_anno_from_ids(self, id_):
        ...
        # support_anno_ids = self.coco.getAnnIds(imgIds=[int(id_)], iscrowd=False)
        # support_annotations = self.coco.loadAnns(support_anno_ids)
        ntarget_img_id = self._get_target_im_id(id_)
        ntarget_anno_ids = self.coco.getAnnIds(imgIds=[ntarget_img_id], iscrowd=False)
        ntarget_annotations = self.coco.loadAnns(ntarget_anno_ids)

        ntarget_objs = []
        for obj1 in ntarget_annotations:
            x1 = np.max((0, obj1["bbox"][0]))
            y1 = np.max((0, obj1["bbox"][1]))
            x2 = np.min((width-1, x1 + np.max((0, obj1["bbox"][2]))))
            y2 = np.min((height-1, y1 + np.max((0, obj1["bbox"][3]))))
            if obj1["area"] > 0 and x2 >= x1 and y2 >= y1:
                obj1["clean_bbox"] = [x1, y1, x2, y2]
                ntarget_objs.append(obj1)

        ntarget_num_objs = len(ntarget_objs)


        ntarget_res = np.zeros((ntarget_num_objs, 5))

        for ix, obj1 in enumerate(ntarget_objs):
            ntarget_cls = self.class_ids.index(obj1["category_id"])
            ntarget_res[ix, 0:4] = obj1["clean_bbox"]
            ntarget_res[ix, 4] = ntarget_cls

        ntarget_r = min(self.img_size[0] / height, self.img_size[1] / width)
        ntarget_res[:, :4] *= ntarget_r
        ...
        return (res, support_res, img_info, resized_info, short_im_annos, long_im_annos, short_file_names, long_file_names, ntarget_res)
    ...
    def pull_item(self, index):
        ...
        res, support_res, img_info, resized_info, _, _, _, _, ntarget_res = self.annotations[index]
        ...
        return short_imgs, long_imgs, res.copy(), support_res.copy(), img_info, np.array([id_]), ntarget_res.copy()
    ...
    def __getitem__(self, index):
        ...
        """
        short_imgs, long_imgs, target, support_target, img_info, img_id, ntarget = self.pull_item(index)

        if self.preproc is not None:

            short_imgs, long_imgs, target, support_target, ntarget = self.preproc(short_imgs, long_imgs, (target, support_target, ntarget), self.input_dim)

        # return np.concatenate((img, support_img), axis=0), (target, support_target), img_info, img_id
        if len(long_imgs) > 0:
            return np.concatenate(short_imgs, axis=0), np.concatenate(long_imgs, axis=0), (target, support_target, ntarget), img_info, img_id
        else: # 不使用long支路的情况
            return np.concatenate(short_imgs, axis=0), np.zeros((0, )), (target, support_target, ntarget), img_info, img_id
