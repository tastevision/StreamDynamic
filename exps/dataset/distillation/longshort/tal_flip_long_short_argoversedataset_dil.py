...
class LONGSHORT_Dil_ARGOVERSEDataset(Dataset):
    """
    COCO dataset class.
    """
    def __init__(self,
    ...
    def _get_im_anno_file_name_same(self, id_):
        ...
        future_file_name = os.path.join(self.data_dir, 'Argoverse-1.1', 'tracking', self.seq_dirs[im_sid], im_name)

        return short_im_annos, long_im_annos, short_file_names, long_file_names, future_file_name
    ...
    def _get_im_anno_file_name_diff(self, id_):
        ...
        cur_im_ann = self.coco.loadImgs(id_ + 1)[0]
        cur_im_name = cur_im_ann['name']
        cur_im_sid = cur_im_ann['sid']
        future_file_name = os.path.join(self.data_dir, 'Argoverse-1.1', 'tracking', self.seq_dirs[cur_im_sid], cur_im_name)
    ...
    def load_anno_from_ids(self, id_):
        ...
        if self.coco.dataset['images'][int(id_)]['fid'] == 0:
            short_im_annos, long_im_annos, short_file_names, long_file_names, future_file_name = \
        ...
        return (res, support_res, img_info, resized_info, short_im_annos, long_im_annos, short_file_names, long_file_names, future_file_name)
    ...
    def load_future_resized_img(self, index):
        img = self.load_future_image(index)
    ...
    def load_future_image(self, index):
    ...
    def pull_item(self, index):
        ...
        res, support_res, img_info, resized_info, _, _, _, _, _ = self.annotations[index]
        ...
        future_img = self.load_future_resized_img(index)

        return short_imgs, long_imgs, future_img, res.copy(), support_res.copy(), img_info, np.array([id_])
    ...
    def __getitem__(self, index):
        ...
        """
        short_imgs, long_imgs, future_img, target, support_target, img_info, img_id = self.pull_item(index)

        if self.preproc is not None:

            short_imgs, long_imgs, future_img, target, support_target = self.preproc(short_imgs, long_imgs, future_img, (target, support_target), self.input_dim)

        # return np.concatenate((img, support_img), axis=0), (target, support_target), img_info, img_id
        if len(long_imgs) > 0:
            return np.concatenate(short_imgs+[future_img], axis=0), np.concatenate(long_imgs, axis=0), (target, support_target), img_info, img_id
        else: # 不使用long支路的情况
            return np.concatenate(short_imgs+[future_img], axis=0), np.zeros((0, )), (target, support_target), img_info, img_id
